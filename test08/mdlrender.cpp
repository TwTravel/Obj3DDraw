...
//################################################################## 
   ...
   camy   = atof( (char*)strvec[2].c_str() );
   camz   = atof( (char*)strvec[3].c_str() );
   campov = atof( (char*)strvec[4].c_str() );  
...
//##################################################################  
   ...
   rot_angle   = atof( (char*)strvec[0].c_str() );
   rot_x = atof( (char*)strvec[1].c_str() );
   rot_y = atof( (char*)strvec[2].c_str() );
   rot_z = atof( (char*)strvec[3].c_str() ); 
   strvec.clear(); splitString(lines[2], strvec, ","); 
   trans_x = atof( (char*)strvec[1].c_str() );
   trans_y = atof( (char*)strvec[2].c_str() );
   trans_z = atof( (char*)strvec[3].c_str() );
...
//##################################################################  
   ...
   for( i = 0; i <  element_num; i++ )
   {
	   ...
	   string  fname;
	   fname = strvec[0];
	   mat_h = atof((char*)strvec[1].c_str());
	   mat_s = atof((char*)strvec[2].c_str());
	   mat_v = atof((char*)strvec[3].c_str());
	   
	   obj_x = atof((char*)strvec[4].c_str());
	   obj_y = atof((char*)strvec[5].c_str());
	   obj_z = atof((char*)strvec[6].c_str());
	   
	   rot_angle =  atof((char*)strvec[ 7].c_str());
	   rot_x     =  atof((char*)strvec[ 8].c_str());
	   rot_y     =  atof((char*)strvec[ 9].c_str());
	   rot_z     =  atof((char*)strvec[10].c_str());
	   obj_scale =  atof((char*)strvec[11].c_str()); 
       ...
       StlElements[i].LoadStl((char*)fname.c_str());
